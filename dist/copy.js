...
function copyWorker(context, configFile) {
    var copyConfig = (0, config_1.fillConfigDefaults)(configFile, exports.taskInfo.defaultConfigFile);
    ...
    return Promise.resolve().then(function () {
        ...
        if ((0, helpers_1.getBooleanPropertyValue)(Constants.ENV_CLEAN_BEFORE_COPY)) {
        ...
        for (var _i = 0, directoryPathList_1 = directoryPathList; _i < directoryPathList_1.length; _i++) {
            ...
            (0, fs_extra_1.mkdirpSync)(directoryPath);
        }
    }).then(function () {
        ...
        var _loop_1 = function (file) {
            cacheCopyData(file);
            var promise = (0, helpers_1.copyFileAsync)(file.absoluteSourcePath, file.absoluteDestPath);
...
function copyUpdate(changedFiles, context) {
    ...
    var configFile = (0, config_1.getUserConfigFile)(context, exports.taskInfo, null);
    var copyConfig = (0, config_1.fillConfigDefaults)(configFile, exports.taskInfo.defaultConfigFile);
    var keys = Object.keys(copyConfig);
    var directoriesToCreate = new Set();
    var toCopyList = [];
    return Promise.resolve().then(function () {
        ...
        additions.forEach(function (changedFile) {
            ...
            matchingItems.forEach(function (matchingItem) {
                ...
                (0, fs_extra_1.mkdirpSync)((0, path_1.dirname)(matchingItem.absoluteDestPath));
                // cache the data and copy the files
                cacheCopyData(matchingItem);
                promises.push((0, helpers_1.copyFileAsync)(matchingItem.absoluteSourcePath, matchingItem.absoluteDestPath));
                (0, events_1.emit)(events_1.EventType.FileChange, additions);
            });
        });
        return Promise.all(promises);
    }).then(function () {
...
function cleanDirectories(context, directoriesToCreate) {
    var filterOut = (0, config_1.replacePathVars)(context, FILTER_OUT_DIRS_FOR_CLEAN);
    var directoryPathList = Array.from(directoriesToCreate);
...
function deleteDirectories(directoryPaths) {
    ...
    for (var _i = 0, directoryPaths_1 = directoryPaths; _i < directoryPaths_1.length; _i++) {
        ...
        promises.push((0, helpers_1.rimRafAsync)(directoryPath));
    }
    return Promise.all(promises);
}
function processRemoveFile(changedFile) {
    ...
    list.forEach(function (copiedFile) {
        var promise = (0, helpers_1.unlinkAsync)(copiedFile.absoluteDestPath);
        promises.push(promise);
    ...
    (0, events_1.emit)(events_1.EventType.FileDelete, deletedFilePaths);
...
function processRemoveDir(changedFile) {
    ...
    copyFilePathCache.forEach(function (copiedFiles, filePath) {
        if ((0, path_1.dirname)(filePath) === changedFile.filePath) {
            keysToRemove.push(filePath);
            copiedFiles.forEach(function (copiedFile) { return directoriesToRemove.add((0, path_1.dirname)(copiedFile.absoluteDestPath)); });
        }
    });
    ...
    directoriesToRemove.forEach(function (directoryToRemove) {
        promises.push((0, helpers_1.rimRafAsync)(directoryToRemove));
    });
    (0, events_1.emit)(events_1.EventType.DirectoryDelete, Array.from(directoriesToRemove));
    return Promise.all(promises);
}
...
function getFilesPathsForConfig(copyConfigKeys, copyConfig) {
    ...
    copyConfigKeys.forEach(function (key) {
        ...
        if (copyOptions && copyOptions.src) {
            var promise = (0, glob_util_1.globAll)(copyOptions.src);
            promises.push(promise);
...
function populateFileAndDirectoryInfo(resultMap, copyConfig, toCopyList, directoriesToCreate) {
    resultMap.forEach(function (globResults, dictionaryKey) {
        globResults.forEach(function (globResult) {
            ...
            var relativePath = (0, path_1.relative)(globResult.base, globResult.absolutePath);
            // append the relative path to the dest
            var destFileName = (0, path_1.resolve)((0, path_1.join)(copyConfig[dictionaryKey].dest, relativePath));
            // store the file information
            toCopyList.push({
                absoluteSourcePath: globResult.absolutePath,
                absoluteDestPath: destFileName
            });
            var directoryToCreate = (0, path_1.dirname)(destFileName);
...
function cleanConfigContent(dictionaryKeys, copyConfig, context) {
    dictionaryKeys.forEach(function (key) {
        ...
        if (copyOption && copyOption.src && copyOption.src.length) {
            var cleanedUpSrc = (0, config_1.replacePathVars)(context, copyOption.src);
            copyOption.src = cleanedUpSrc;
            var cleanedUpDest = (0, config_1.replacePathVars)(context, copyOption.dest);
...
function copyConfigToWatchConfig(context) {
    ...
    var configFile = (0, config_1.getUserConfigFile)(context, exports.taskInfo, '');
    var copyConfig = (0, config_1.fillConfigDefaults)(configFile, exports.taskInfo.defaultConfigFile);
    var results = [];
    for (var _i = 0, _a = Object.keys(copyConfig); _i < _a.length; _i++) {
        var key = _a[_i];
        if (copyConfig[key] && copyConfig[key].src) {
            var list = (0, glob_util_1.generateGlobTasks)(copyConfig[key].src, {});
