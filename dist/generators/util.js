...
function hydrateRequest(context, request) {
    ...
    hydrated.className = (0, helpers_3.ensureSuffix)((0, helpers_2.pascalCase)(request.name), (0, helpers_2.upperCaseFirst)(suffix));
    hydrated.fileName = (0, helpers_3.removeSuffix)((0, helpers_2.paramCase)(request.name), "-" + (0, helpers_2.paramCase)(suffix));
    if (request.type === 'pipe')
        hydrated.pipeName = (0, helpers_2.camelCase)(request.name);
    ...
    hydrated.dirToRead = (0, path_1.join)((0, helpers_2.getStringPropertyValue)(Constants.ENV_VAR_IONIC_ANGULAR_TEMPLATE_DIR), request.type);
    var baseDir = getDirToWriteToByType(context, request.type);
    hydrated.dirToWrite = (0, path_1.join)(baseDir, hydrated.fileName);
    return hydrated;
}
...
function createCommonModule(envVar, requestType) {
    ...
    (0, fs_1.writeFileSync)(envVar, tmplt);
}
...
function hydrateTabRequest(context, request) {
    ...
    for (var i = 0; i < request.tabs.length; i++) {
        var tabVar = (0, helpers_2.camelCase)(request.tabs[i].name) + "Root";
        if (hydrated.includeNgModule) {
        ...
        if (i === request.tabs.length - 1) {
            hydrated.tabContent += "    <ion-tab [root]=\"" + tabVar + "\" tabTitle=\"" + (0, helpers_2.sentenceCase)(request.tabs[i].name) + "\" tabIcon=\"information-circle\"></ion-tab>";
        }
        else {
            hydrated.tabContent += "    <ion-tab [root]=\"" + tabVar + "\" tabTitle=\"" + (0, helpers_2.sentenceCase)(request.tabs[i].name) + "\" tabIcon=\"information-circle\"></ion-tab>\n";
        }
...
function readTemplates(pathToRead) {
    var fileNames = (0, fs_1.readdirSync)(pathToRead);
    var absolutePaths = fileNames.map(function (fileName) {
        return (0, path_1.join)(pathToRead, fileName);
    });
    var filePathToContent = new Map();
    var promises = absolutePaths.map(function (absolutePath) {
        var promise = (0, helpers_2.readFileAsync)(absolutePath);
...
function filterOutTemplates(request, templates) {
    ...
    templates.forEach(function (fileContent, filePath) {
        var newFileExtension = (0, path_1.basename)(filePath, GeneratorConstants.KNOWN_FILE_EXTENSION);
...
function applyTemplates(request, templates) {
    ...
    templates.forEach(function (fileContent, filePath) {
        fileContent = (0, helpers_2.replaceAll)(fileContent, GeneratorConstants.CLASSNAME_VARIABLE, request.className);
        fileContent = (0, helpers_2.replaceAll)(fileContent, GeneratorConstants.PIPENAME_VARIABLE, request.pipeName);
        fileContent = (0, helpers_2.replaceAll)(fileContent, GeneratorConstants.IMPORTSTATEMENT_VARIABLE, request.importStatement);
        fileContent = (0, helpers_2.replaceAll)(fileContent, GeneratorConstants.IONICPAGE_VARIABLE, request.ionicPage);
        fileContent = (0, helpers_2.replaceAll)(fileContent, GeneratorConstants.FILENAME_VARIABLE, request.fileName);
        fileContent = (0, helpers_2.replaceAll)(fileContent, GeneratorConstants.SUPPLIEDNAME_VARIABLE, request.name);
        fileContent = (0, helpers_2.replaceAll)(fileContent, GeneratorConstants.TAB_CONTENT_VARIABLE, request.tabContent);
        fileContent = (0, helpers_2.replaceAll)(fileContent, GeneratorConstants.TAB_VARIABLES_VARIABLE, request.tabVariables);
        fileContent = (0, helpers_2.replaceAll)(fileContent, GeneratorConstants.TABS_IMPORTSTATEMENT_VARIABLE, request.tabsImportStatement);
...
function writeGeneratedFiles(request, processedTemplates) {
    ...
    processedTemplates.forEach(function (fileContent, filePath) {
        ...
        var fileToWrite = (0, path_1.join)(request.dirToWrite, newFileName);
...
function createDirAndWriteFile(filePath, fileContent) {
    var directory = (0, path_1.dirname)(filePath);
    return (0, helpers_2.mkDirpAsync)(directory).then(function () {
        return (0, helpers_2.writeFileAsync)(filePath, fileContent);
    });
}
function getNgModules(context, types) {
    var ngModuleSuffix = (0, helpers_2.getStringPropertyValue)(Constants.ENV_NG_MODULE_FILE_NAME_SUFFIX);
    var patterns = types.map(function (type) { return (0, path_1.join)(getDirToWriteToByType(context, type), '**', "*" + ngModuleSuffix); });
    return (0, glob_util_1.globAll)(patterns);
}
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    envVar = (0, helpers_2.getStringPropertyValue)("IONIC_" + hydratedRequest.type.toUpperCase() + "S_NG_MODULE_PATH");
                ...
                case 1:
                    ...
                    if (hydratedRequest.type === 'pipe' || hydratedRequest.type === 'component' || hydratedRequest.type === 'directive') {
                        if (!(0, fs_1.existsSync)(envVar))
                            createCommonModule(envVar, hydratedRequest.type);
                    }
                    typescriptFilePath = (0, helpers_3.changeExtension)(templatesArray.filter(function (path) { return (0, path_1.extname)(path) === '.ts'; })[0], '');
                    (0, helpers_2.readFileAsync)(ngModulePath).then(function (content) {
                        importPath = type === 'pipe' || type === 'component' || type === 'directive'
                            ...
                            ? (0, helpers_1.toUnixPath)("./" + (0, path_1.relative)((0, path_1.dirname)(ngModulePath), hydratedRequest.dirToWrite) + path_1.sep + hydratedRequest.fileName)
                            : (0, helpers_1.toUnixPath)("" + (0, path_1.relative)((0, path_1.dirname)(ngModulePath), hydratedRequest.dirToWrite) + path_1.sep + hydratedRequest.fileName);
                        content = (0, typescript_utils_1.insertNamedImportIfNeeded)(ngModulePath, content, hydratedRequest.className, importPath);
                        if (type === 'pipe' || type === 'component' || type === 'directive') {
                            content = (0, typescript_utils_1.appendNgModuleDeclaration)(ngModulePath, content, hydratedRequest.className);
                            content = (0, typescript_utils_1.appendNgModuleExports)(ngModulePath, content, hydratedRequest.className);
                        }
                        if (type === 'provider') {
                            content = (0, typescript_utils_1.appendNgModuleProvider)(ngModulePath, content, hydratedRequest.className);
                        }
                        return (0, helpers_2.writeFileAsync)(ngModulePath, content);
                    });
                    return [2 /*return*/];
            }
        });
...
function tabsModuleManipulation(tabs, hydratedRequest, tabHydratedRequests) {
    ...
    if (!ngModulePath) {
        // Static imports
        var tabsPath_1 = (0, path_1.join)(hydratedRequest.dirToWrite, hydratedRequest.fileName + ".ts");
        var modifiedContent_1 = null;
        return (0, helpers_2.readFileAsync)(tabsPath_1).then(function (content) {
            tabHydratedRequests.forEach(function (tabRequest) {
                var typescriptFilePath = (0, helpers_3.changeExtension)(tabRequest.generatedFileNames.filter(function (path) { return (0, path_1.extname)(path) === '.ts'; })[0], '');
                var importPath = (0, helpers_1.toUnixPath)((0, path_1.relative)((0, path_1.dirname)(tabsPath_1), typescriptFilePath));
                modifiedContent_1 = (0, typescript_utils_1.insertNamedImportIfNeeded)(tabsPath_1, content, tabRequest.className, importPath);
                content = modifiedContent_1;
            });
            return (0, helpers_2.writeFileAsync)(tabsPath_1, modifiedContent_1);
        });
    }
}
...
function generateTemplates(context, request, includePageConstants) {
    ...
    var pageConstantFile = (0, path_1.join)(context.pagesDir, 'pages.constants.ts');
    if (includePageConstants && !(0, fs_1.existsSync)(pageConstantFile))
...
function createConstStatments(pageConstantFile, request) {
    (0, helpers_2.readFileAsync)(pageConstantFile).then(function (content) {
        content += "\nexport const " + (0, helpers_2.constantCase)(request.className) + " = '" + request.className + "';";
        (0, helpers_2.writeFileAsync)(pageConstantFile, content);
    });
}
exports.createConstStatments = createConstStatments;
function createPageConstants(context) {
    var pageConstantFile = (0, path_1.join)(context.pagesDir, 'pages.constants.ts');
    (0, helpers_2.writeFileAsync)(pageConstantFile, '//Constants for getting type references');
}
