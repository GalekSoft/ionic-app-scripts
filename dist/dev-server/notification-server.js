...
function createNotificationServer(config) {
    ...
    (0, events_1.on)(events_1.EventType.BuildUpdateStarted, function (buildUpdateMsg) {
        var msg = {
    ...
    (0, events_1.on)(events_1.EventType.BuildUpdateCompleted, function (buildUpdateMsg) {
        var msg = {
            category: 'buildUpdate',
            type: 'completed',
            data: {
                buildId: buildUpdateMsg.buildId,
                reloadApp: buildUpdateMsg.reloadApp,
                diagnosticsHtml: (0, logger_diagnostics_1.hasDiagnostics)(config.buildDir) ? (0, logger_diagnostics_1.getDiagnosticsHtmlContent)(config.buildDir) : null
            }
        };
        queueMessageSend(msg);
    });
    ...
    function handleRuntimeError(clientMsg) {
        var msg = {
            ...
            type: 'completed',
            data: {
                diagnosticsHtml: (0, logger_runtime_1.generateRuntimeDiagnosticContent)(config.rootDir, config.buildDir, clientMsg.data.message, clientMsg.data.stack)
            }
        };
        queueMessageSend(msg);
    }
