...
function setupProxies(app) {
    if ((0, helpers_1.getBooleanPropertyValue)(Constants.ENV_READ_CONFIG_JSON)) {
        (0, ionic_project_1.getProjectJson)().then(function (projectConfig) {
...
function serveIndex(req, res) {
    ...
    fs.readFile(indexFileName, function (err, indexHtml) {
        ...
        if (config.useLiveReload) {
            indexHtml = (0, live_reload_1.injectLiveReloadScript)(indexHtml, req.hostname, config.liveReloadPort);
            indexHtml = (0, injector_1.injectNotificationScript)(config.rootDir, indexHtml, config.notifyOnConsoleLog, config.notificationPort);
        }
        indexHtml = (0, logger_diagnostics_1.injectDiagnosticsHtml)(config.buildDir, indexHtml);
