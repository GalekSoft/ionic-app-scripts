...
function startWatchers(context, configFile) {
    var watchConfig = (0, config_1.fillConfigDefaults)(configFile, taskInfo.defaultConfigFile);
    var promises = [];
...
function startWatcher(name, watcher, context) {
    return new Promise(function (resolve, reject) {
        ...
        }, (0, helpers_1.getIntPropertyValue)(Constants.ENV_START_WATCH_TIMEOUT));
        ...
        chokidarWatcher.on(eventName, function (event, filePath) {
            ...
            filePath = (0, path_1.normalize)((0, path_1.resolve)((0, path_1.join)(context.rootDir, filePath)));
...
function prepareWatcher(context, watcher) {
    ...
    if (watcher.options.ignored) {
        if (Array.isArray(watcher.options.ignored)) {
            watcher.options.ignored = watcher.options.ignored.map(function (p) { return (0, path_1.normalize)((0, config_1.replacePathVars)(context, p)); });
        }
        else if (typeof watcher.options.ignored === 'string') {
            // it's a string, so just do it once and leave it
            watcher.options.ignored = (0, path_1.normalize)((0, config_1.replacePathVars)(context, watcher.options.ignored));
        }
    }
    if (watcher.paths) {
        if (Array.isArray(watcher.paths)) {
            watcher.paths = watcher.paths.map(function (p) { return (0, path_1.normalize)((0, config_1.replacePathVars)(context, p)); });
        }
        else {
            watcher.paths = (0, path_1.normalize)((0, config_1.replacePathVars)(context, watcher.paths));
        }
    }
}
...
function copyUpdate(event, filePath, context) {
    ...
    if (!queuedCopyChanges.some(function (f) { return f.filePath === filePath; })) {
        ...
        queuedCopyTimerId = setTimeout(function () {
            ...
            if (changedFiles && changedFiles.length) {
                // cool, we've got some build updating to do ;)
                (0, copy_1.copyUpdate)(changedFiles, context);
            }
        }, BUILD_UPDATE_DEBOUNCE_MS);
    }
    return Promise.resolve();
...
function runBuildUpdate(context, changedFiles) {
    ...
    if (tsFiles.length) {
        ...
        for (var _i = 0, tsFiles_1 = tsFiles; _i < tsFiles_1.length; _i++) {
            ...
            if (!(0, transpile_1.canRunTranspileUpdate)(tsFile.event, tsFiles[0].filePath, context)) {
