...
exports.getSassConfig = exports.sassWorker = exports.sassUpdate = exports.sass = void 0;
...
function sassUpdate(changedFiles, context) {
    var configFile = (0, config_1.getUserConfigFile)(context, taskInfo, null);
...
function sassWorker(context, configFile) {
    ...
    if (!context.moduleFiles && !sassConfig.file) {
        ...
        bundlePromise.push((0, bundle_1.bundle)(context));
    }
    return Promise.all(bundlePromise).then(function () {
        (0, logger_diagnostics_1.clearDiagnostics)(context, logger_diagnostics_1.DiagnosticsType.Sass);
        // where the final css output file is saved
        if (!sassConfig.outFile) {
            sassConfig.outFile = (0, path_1.join)(context.buildDir, sassConfig.outputFilename);
        }
        logger_1.Logger.debug("sass outFile: " + sassConfig.outFile);
        // import paths where the sass compiler will look for imports
        sassConfig.includePaths.unshift((0, path_1.join)(context.srcDir));
        ...
        if (!sassConfig.file) {
            ...
            sassConfig.file = (0, config_1.replacePathVars)(context, sassConfig.file);
        }
...
function getSassConfig(context, configFile) {
    configFile = (0, config_1.getUserConfigFile)(context, taskInfo, configFile);
    return (0, config_1.fillConfigDefaults)(configFile, taskInfo.defaultConfigFile);
}
...
function generateSassData(context, sassConfig) {
    ...
    if (context.moduleFiles) {
        context.moduleFiles.forEach(function (moduleFile) {
            var moduleDirectory = (0, path_1.dirname)(moduleFile);
    ...
    var userSassVariableFiles = sassConfig.variableSassFiles.map(function (f) {
        return (0, config_1.replacePathVars)(context, f);
    });
    ...
    if (!siblingFiles.length && componentPath.indexOf(path_1.sep + 'node_modules') === -1) {
        ...
        for (var k in sassConfig.directoryMaps) {
            if (sassConfig.directoryMaps.hasOwnProperty(k)) {
                var actualDirectory = (0, config_1.replacePathVars)(context, k);
                var mappedDirectory = (0, config_1.replacePathVars)(context, sassConfig.directoryMaps[k]);
...
function getSiblingSassFiles(componentPath, sassConfig) {
    try {
        return (0, fs_extra_1.readdirSync)(componentPath).filter(function (f) {
            return isValidSassFile(f, sassConfig);
        }).map(function (f) {
            return (0, path_1.join)(componentPath, f);
        });
    }
    catch (ex) {
...
function render(context, sassConfig) {
    return new Promise(function (resolve, reject) {
        ...
        (0, node_sass_1.render)(sassConfig, function (sassError, sassResult) {
            var diagnostics = (0, logger_sass_1.runSassDiagnostics)(context, sassError);
            if (diagnostics.length) {
                (0, logger_diagnostics_1.printDiagnostics)(context, logger_diagnostics_1.DiagnosticsType.Sass, diagnostics, true, true);
                // sass render error :(
...
function renderSassSuccess(context, sassResult, sassConfig) {
    if (sassConfig.autoprefixer) {
        ...
        var postcssOptions = {
            to: (0, path_1.basename)(sassConfig.outFile),
        ...
        if (sassConfig.postCssPlugins) {
            postCssPlugins = __spreadArray(__spreadArray([], sassConfig.postCssPlugins, true), postCssPlugins, true);
        }
...
function generateSourceMaps(sassResult, sassConfig) {
    ...
    if (sassResult.map) {
        ...
        var sassFileSrcPath_1 = (0, path_1.dirname)(sassFileSrc);
        if (sassFileSrcPath_1) {
            ...
            sassMap.sources = sassMap.sources.map(function (source, index) {
                return (index === sourceFileIndex_1) ? source : (0, path_1.join)(sassFileSrcPath_1, source);
            });
        }
...
function writeOutput(context, sassConfig, cssOutput, sourceMap) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        var buildDir = (0, path_1.dirname)(sassConfig.outFile);
        (0, fs_extra_1.ensureDirSync)(buildDir);
        (0, fs_extra_1.writeFile)(sassConfig.outFile, cssOutput, function (cssWriteErr) {
            if (cssWriteErr) {
                ...
                if (mappingsOutput) {
                    ...
                    var sourceMapPath_1 = (0, path_1.join)(buildDir, (0, path_1.basename)(sassConfig.outFile) + '.map');
                    logger_1.Logger.debug("sass start write css map: " + sourceMapPath_1);
                    (0, fs_extra_1.writeFile)(sourceMapPath_1, mappingsOutput, function (mapWriteErr) {
                        if (mapWriteErr) {
