...
function webpackUpdate(changedFiles, context, configFile) {
    ...
    (0, events_2.emit)(events_2.EventType.WebpackFilesChanged, null);
...
function webpackBuildComplete(stats, context, webpackConfig) {
    if ((0, helpers_1.getBooleanPropertyValue)(Constants.ENV_PRINT_WEBPACK_DEPENDENCY_TREE)) {
        logger_1.Logger.debug('Webpack Dependency Map Start');
        var dependencyMap = (0, helpers_1.webpackStatsToDependencyMap)(context, stats);
        (0, helpers_1.printDependencyMap)(dependencyMap);
...
function setBundledFiles(context) {
    var bundledFilesToWrite = context.fileCache.getAll().filter(function (file) {
        return (0, path_1.dirname)(file.path).indexOf(context.buildDir) >= 0 && (file.path.endsWith('.js') || file.path.endsWith('.js.map'));
    });
...
function getWebpackConfig(context, configFile) {
    configFile = (0, config_1.getUserConfigFile)(context, taskInfo, configFile);
    var webpackConfigDictionary = (0, config_1.fillConfigDefaults)(configFile, taskInfo.defaultConfigFile);
    var webpackConfig = getWebpackConfigFromDictionary(context, webpackConfigDictionary);
    webpackConfig.entry = (0, config_1.replacePathVars)(context, webpackConfig.entry);
    webpackConfig.output.path = (0, config_1.replacePathVars)(context, webpackConfig.output.path);
...
function getOutputDest(context) {
    ...
    return (0, path_1.join)(webpackConfig.output.path, webpackConfig.output.filename);
}
