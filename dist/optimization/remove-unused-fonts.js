...
function removeUnusedFonts(context) {
    var fontDir = (0, helpers_1.getStringPropertyValue)(Constants.ENV_VAR_FONTS_DIR);
    return (0, helpers_1.readDirAsync)(fontDir).then(function (fileNames) {
        fileNames = fileNames.sort();
        var toPurge = getFontFileNamesToPurge(context.target, context.platform, fileNames);
        var fullPaths = toPurge.map(function (fileName) { return (0, path_1.join)(fontDir, fileName); });
        var promises = fullPaths.map(function (fullPath) { return (0, helpers_1.unlinkAsync)(fullPath); });
...
function isWoof(fileName) {
    return (0, path_1.extname)(fileName) === '.woff' || (0, path_1.extname)(fileName) === '.woff2';
}
