...
function updateIndexHtml(context) {
    var indexPath = (0, path_1.join)(context.wwwDir, context.wwwIndex);
    return (0, helpers_1.readFileAsync)(indexPath).then(function (indexHtml) {
        if (!indexHtml) {
            return Promise.resolve(null);
        }
        indexHtml = injectCoreScripts(context, indexHtml);
        return (0, helpers_1.writeFileAsync)(indexPath, indexHtml);
    });
}
...
function injectCoreScripts(context, indexHtml) {
    ...
    inject.push("    " + (0, ionic_global_1.buildIonicGlobal)(context));
    inject.push("  </script>");
