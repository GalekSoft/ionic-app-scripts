...
function scanSrcTsFiles(context) {
    var srcGlob = (0, path_1.join)(context.srcDir, '**', '*.ts');
    var globs = [srcGlob];
    var deepLinkDir = (0, helpers_1.getStringPropertyValue)(Constants.ENV_VAR_DEEPLINKS_DIR);
    ...
    if (deepLinkDir !== context.srcDir) {
        globs.push((0, path_1.join)(deepLinkDir, '**', '*.ts'));
    }
    return (0, glob_util_1.globAll)(globs).then(function (results) {
        var promises = results.map(function (result) {
            var promise = (0, helpers_1.readFileAsync)(result.absolutePath);
...
function validateRequiredFilesExist(context) {
    return Promise.all([
        (0, helpers_1.readFileAsync)(process.env[Constants.ENV_APP_ENTRY_POINT]),
        (0, transpile_1.getTsConfigAsync)(context, process.env[Constants.ENV_TS_CONFIG])
        ...
        if (error.code === 'ENOENT' && error.path === process.env[Constants.ENV_TS_CONFIG]) {
            error = new errors_1.BuildError([error.path + " was not found. The \"tsconfig.json\" file is missing. This file is required.", 'For more information please see the default Ionic project tsconfig.json file here:',
                'https://github.com/ionic-team/ionic2-app-base/blob/master/tsconfig.json'].join('\n'));
            error.isFatal = true;
            throw error;
        }
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    if (!(0, helpers_1.getBooleanPropertyValue)(Constants.ENV_SKIP_IONIC_ANGULAR_VERSION)) {
                ...
                case 1:
                    versions = _a.sent();
                    context.angularVersion = (0, helpers_1.semverStringToObject)(versions[0]);
                    if (!(0, helpers_1.getBooleanPropertyValue)(Constants.ENV_SKIP_IONIC_ANGULAR_VERSION)) {
                        context.ionicAngularVersion = (0, helpers_1.semverStringToObject)(versions[1]);
                    }
                    // index could be 1 or 2 depending on if you read ionic-angular, its always the last one bro
                    context.typescriptVersion = (0, helpers_1.semverStringToObject)(versions[versions.length - 1]);
                    return [2 /*return*/];
            }
        });
    });
...
function readPackageVersion(packageDir) {
    return __awaiter(this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    packageJsonPath = (0, path_1.join)(packageDir, 'package.json');
                    return [4 /*yield*/, (0, helpers_1.readJsonAsync)(packageJsonPath)];
                case 1:
