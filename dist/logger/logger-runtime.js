...
function generateRuntimeDiagnosticContent(rootDir, buildDir, runtimeErrorMessage, runtimeErrorStack) {
    ...
    if (runtimeErrorMessage) {
        ...
        c.push("<div class=\"ion-diagnostic-message\">" + (0, helpers_1.escapeHtml)(runtimeErrorMessage) + "</div>");
    }
    ...
    diagnosticsHtmlCache.forEach(function (d) {
        c.push((0, logger_diagnostics_1.generateCodeBlock)(d));
    });
    if (runtimeErrorStack) {
        c.push("<div class=\"ion-diagnostic-stack-header\">Stack</div>");
        c.push("<div class=\"ion-diagnostic-stack\">" + (0, helpers_1.escapeHtml)(runtimeErrorStack) + "</div>");
    }
    c.push("</div>"); // .ion-diagnostic
    return (0, logger_diagnostics_1.getDiagnosticsHtmlContent)(buildDir, c.join('\n'));
}
...
function generateRuntimeStackDiagnostics(rootDir, stack) {
    var diagnostics = [];
    if (stack) {
        (0, helpers_1.splitLineBreaks)(stack).forEach(function (stackLine) {
            try {
                ...
                var d = {
                    ...
                    code: 'runtime',
                    messageText: '',
                    absFileName: (0, path_1.resolve)(rootDir, fileName),
                    relFileName: (0, path_1.normalize)(fileName),
                    lines: []
                };
                var sourceText = (0, fs_1.readFileSync)(d.absFileName, 'utf8');
                var srcLines = (0, helpers_1.splitLineBreaks)(sourceText);
                if (!srcLines.length || errorLineNumber >= srcLines.length)
                    return;
                var htmlLines = srcLines;
                try {
                    htmlLines = (0, helpers_1.splitLineBreaks)((0, highlight_1.highlight)(d.language, sourceText, true).value);
                }
                catch (e) { }
                var errorLine = {
                ...
                if (errorLine.html.indexOf('class="hljs') === -1) {
                    try {
                        errorLine.html = (0, highlight_1.highlight)(d.language, errorLine.text, true).value;
                    }
                    catch (e) { }
                }
                ...
                if (errorLine.lineIndex > 0) {
                    ...
                    if (previousLine.html.indexOf('class="hljs') === -1) {
                        try {
                            previousLine.html = (0, highlight_1.highlight)(d.language, previousLine.text, true).value;
                        }
                        catch (e) { }
                    }
                ...
                if (errorLine.lineIndex < srcLines.length) {
                    ...
                    if (nextLine.html.indexOf('class="hljs') === -1) {
                        try {
                            nextLine.html = (0, highlight_1.highlight)(d.language, nextLine.text, true).value;
                        }
                        catch (e) { }
                    }
