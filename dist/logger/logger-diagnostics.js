...
function printDiagnostics(context, diagnosticsType, diagnostics, consoleLogDiagnostics, writeHtmlDiagnostics) {
    if (diagnostics && diagnostics.length) {
        ...
        if (writeHtmlDiagnostics) {
            ...
            (0, fs_1.writeFileSync)(fileName, content.join('\n'), { encoding: 'utf8' });
        }
    }
}
...
function clearDiagnostics(context, type) {
    try {
        ...
        (0, fs_1.unlinkSync)(getDiagnosticsFileName(context.buildDir, type));
    }
    catch (e) { }
}
...
function loadBuildDiagnosticsHtml(buildDir) {
    try {
        if (diagnosticsHtmlCache[exports.DiagnosticsType.TypeScript] === undefined) {
            diagnosticsHtmlCache[exports.DiagnosticsType.TypeScript] = (0, fs_1.readFileSync)(getDiagnosticsFileName(buildDir, exports.DiagnosticsType.TypeScript), 'utf8');
        }
    }
    catch (e) {
    ...
    try {
        if (diagnosticsHtmlCache[exports.DiagnosticsType.Sass] === undefined) {
            diagnosticsHtmlCache[exports.DiagnosticsType.Sass] = (0, fs_1.readFileSync)(getDiagnosticsFileName(buildDir, exports.DiagnosticsType.Sass), 'utf8');
        }
    }
    catch (e) {
...
function generateDiagnosticHtml(d) {
    ...
    c.push("<div class=\"ion-diagnostic-masthead\" title=\"" + (0, helpers_1.escapeHtml)(d.type) + " error: " + (0, helpers_1.escapeHtml)(d.code) + "\">");
    var title = (0, helpers_1.titleCase)(d.type) + " " + (0, helpers_1.titleCase)(d.level);
    c.push("<div class=\"ion-diagnostic-title\">" + (0, helpers_1.escapeHtml)(title) + "</div>");
    c.push("<div class=\"ion-diagnostic-message\" data-error-code=\"" + (0, helpers_1.escapeHtml)(d.type) + "-" + (0, helpers_1.escapeHtml)(d.code) + "\">" + (0, helpers_1.escapeHtml)(d.messageText) + "</div>");
...
function generateCodeBlock(d) {
    ...
    c.push("<div class=\"ion-diagnostic-file-header\" title=\"" + (0, helpers_1.escapeHtml)(d.absFileName) + "\">" + (0, helpers_1.escapeHtml)(d.relFileName) + "</div>");
    if (d.lines && d.lines.length) {
        ...
        prepareLines(d.lines, 'html').forEach(function (l) {
            ...
            c.push("<td class=\"ion-diagnostic-blob-code\">" + (0, highlight_1.highlightError)(l.html, l.errorCharStart, l.errorLength) + "</td>");
            c.push("</tr>");
        });
        c.push("</table>");
...
function getDiagnosticsFileName(buildDir, type) {
    return (0, path_1.join)(buildDir, ".ion-diagnostic-" + type + ".html");
}
