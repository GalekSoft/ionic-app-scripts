...
function templateUpdate(changedFiles, context) {
    try {
        ...
        var start = Date.now();
        var bundleFiles = context.fileCache.getAll().filter(function (file) { return file.path.indexOf(context.buildDir) >= 0 && (0, path_1.extname)(file.path) === '.js'; });
        ...
        for (var _i = 0, changedTemplates_1 = changedTemplates; _i < changedTemplates_1.length; _i++) {
            ...
            for (var _a = 0, bundleFiles_1 = bundleFiles; _a < bundleFiles_1.length; _a++) {
                ...
                if (newContent && newContent !== bundleFile.content) {
                    ...
                    (0, fs_1.writeFileSync)(bundleFile.path, newContent);
                }
            }
        }
...
function updateCorrespondingJsFile(context, newTemplateContent, existingHtmlTemplatePath) {
    var moduleFileExtension = (0, helpers_1.changeExtension)((0, helpers_1.getStringPropertyValue)(Constants.ENV_NG_MODULE_FILE_NAME_SUFFIX), '.js');
    var javascriptFiles = context.fileCache.getAll().filter(function (file) { return (0, path_1.dirname)(file.path) === (0, path_1.dirname)(existingHtmlTemplatePath) && (0, path_1.extname)(file.path) === '.js' && !file.path.endsWith(moduleFileExtension); });
    for (var _i = 0, javascriptFiles_1 = javascriptFiles; _i < javascriptFiles_1.length; _i++) {
        ...
        if (newContent && newContent !== javascriptFile.content) {
            ...
            var typescriptFilePath = (0, helpers_1.changeExtension)(javascriptFile.path, '.ts');
...
function inlineTemplate(sourceText, sourcePath) {
    var componentDir = (0, path_1.parse)(sourcePath).dir;
    var match;
    var replacement;
    var lastMatch = null;
...
function updateTemplate(componentDir, match) {
    var htmlFilePath = (0, path_1.join)(componentDir, match.templateUrl);
    try {
        var templateContent = (0, fs_1.readFileSync)(htmlFilePath, 'utf8');
...
function getTemplatePrefix(htmlFilePath) {
    return "template:/*ion-inline-start:\"" + (0, path_1.resolve)(htmlFilePath) + "\"*/";
}
function getTemplateSuffix(htmlFilePath) {
    return "/*ion-inline-end:\"" + (0, path_1.resolve)(htmlFilePath) + "\"*/";
}
