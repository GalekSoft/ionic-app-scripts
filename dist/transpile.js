...
function transpileWorker(context, workerConfig) {
    ...
    return new Promise(function (resolve, reject) {
        (0, logger_diagnostics_1.clearDiagnostics)(context, logger_diagnostics_1.DiagnosticsType.TypeScript);
        ...
        if (workerConfig.sourceMaps === false) {
            ...
            tsConfig.options.sourceMap = (0, bundle_1.buildJsSourceMaps)(context);
        }
        ...
        var host = (0, compiler_host_factory_1.getFileSystemCompilerHostInstance)(tsConfig.options);
        if (workerConfig.useTransforms && (0, helpers_1.getBooleanPropertyValue)(Constants.ENV_PARSE_DEEPLINKS)) {
            ...
            var deepLinkFiles = (0, util_1.filterTypescriptFilesForDeepLinks)(context.fileCache);
            deepLinkFiles.forEach(function (file) {
                file.content = (0, util_1.purgeDeepLinkDecorator)(file.content);
            });
            var file = context.fileCache.get((0, helpers_1.getStringPropertyValue)(Constants.ENV_APP_NG_MODULE_PATH));
            var hasExisting = (0, util_1.hasExistingDeepLinkConfig)(file.path, file.content);
            if (!hasExisting) {
                var deepLinkString = (0, util_1.convertDeepLinkConfigEntriesToString)((0, helpers_1.getParsedDeepLinkConfig)());
                file.content = (0, util_1.getUpdatedAppNgModuleContentWithDeepLinkConfig)(file.path, file.content, deepLinkString);
            }
        }
        ...
        var diagnostics = (0, logger_typescript_1.runTypeScriptDiagnostics)(context, tsDiagnostics);
        if (diagnostics.length) {
            // darn, we've got some things wrong, transpile failed :(
            (0, logger_diagnostics_1.printDiagnostics)(context, logger_diagnostics_1.DiagnosticsType.TypeScript, diagnostics, true, true);
...
function transpileUpdateWorker(event, filePath, context, workerConfig) {
    try {
        ...
        // build the ts source maps if the bundler is going to use source maps
        cachedTsConfig.options.sourceMap = (0, bundle_1.buildJsSourceMaps)(context);
        var beforeArray = [];
        ...
        var textToTranspile = workerConfig.useTransforms && (0, helpers_1.getBooleanPropertyValue)(Constants.ENV_PARSE_DEEPLINKS) ? transformSource(filePath, sourceText) : sourceText;
        // transpile this one module
        var transpileOutput = ts.transpileModule(textToTranspile, transpileOptions);
        var diagnostics = (0, logger_typescript_1.runTypeScriptDiagnostics)(context, transpileOutput.diagnostics);
        if (diagnostics.length) {
            (0, logger_diagnostics_1.printDiagnostics)(context, logger_diagnostics_1.DiagnosticsType.TypeScript, diagnostics, false, true);
            ...
            var newPath = (0, helpers_1.changeExtension)(filePath, '.js');
            ...
            if (workerConfig.inlineTemplate) {
                ...
                jsContent = (0, template_1.inlineTemplate)(transpileOutput.outputText, filePath);
            }
...
function runDiagnosticsWorker(context) {
    if (!diagnosticsWorker) {
        ...
        diagnosticsWorker = (0, child_process_1.fork)(workerModule, [], { env: { FORCE_COLOR: true } });
...
function writeTranspiledFilesCallback(fileCache, sourcePath, data, shouldInlineTemplate) {
    ...
    if (sourcePath.endsWith('.js')) {
        ...
        if (shouldInlineTemplate) {
            file.content = (0, template_1.inlineTemplate)(data, sourcePath);
        }
        else {
            file.content = data;
        }
...
function getTsConfig(context, tsConfigPath) {
    ...
    var tsConfigFile = ts.readConfigFile(tsConfigPath, function (path) { return (0, fs_1.readFileSync)(path, 'utf8'); });
    if (!tsConfigFile) {
        ...
        var diagnostics = (0, logger_typescript_1.runTypeScriptDiagnostics)(context, parsedConfig.errors);
        if (diagnostics.length) {
            (0, logger_diagnostics_1.printDiagnostics)(context, logger_diagnostics_1.DiagnosticsType.TypeScript, diagnostics, true, true);
            throw new errors_1.BuildError("tsconfig: invalid config, \"" + tsConfigPath + "\"\"");
        }
...
function transformSource(filePath, input) {
    if ((0, util_1.isDeepLinkingFile)(filePath)) {
        input = (0, util_1.purgeDeepLinkDecorator)(input);
    }
    else if (filePath === (0, helpers_1.getStringPropertyValue)(Constants.ENV_APP_NG_MODULE_PATH) && !(0, util_1.hasExistingDeepLinkConfig)(filePath, input)) {
        var deepLinkString = (0, util_1.convertDeepLinkConfigEntriesToString)((0, helpers_1.getParsedDeepLinkConfig)());
        input = (0, util_1.getUpdatedAppNgModuleContentWithDeepLinkConfig)(filePath, input, deepLinkString);
    }
    return input;
}
exports.transformSource = transformSource;
function copyOriginalSourceFiles(fileCache) {
    var deepLinkFiles = (0, util_1.filterTypescriptFilesForDeepLinks)(fileCache);
    var appNgModule = fileCache.get((0, helpers_1.getStringPropertyValue)(Constants.ENV_APP_NG_MODULE_PATH));
...
function resetSourceFiles(fileCache) {
    fileCache.getAll().forEach(function (file) {
        if (path.extname(file.path) === ".ts" + exports.inMemoryFileCopySuffix) {
            var originalExtension = (0, helpers_1.changeExtension)(file.path, '.ts');
